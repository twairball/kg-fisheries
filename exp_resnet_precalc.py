...
def calc_train_da_feats():
    ...
    for aug in range(nb_augm):
        ...
        # save
        labels_file = "data/results/resnet_da%d_conv_labels.h5" % aug
        feats_file = "data/results/resnet_da%d_conv_feats.h5" % aug
...
def calc_val_feats():
    ...
    # save
    labels_file = "data/results/resnet_conv_val_labels.h5" 
    feats_file = "data/results/resnet_conv_val_feats.h5"
    save_array(labels_file, labels)
    save_array(feats_file, feats)
    print("    (precalc) feats: %s" % (feats.shape,))
    print("    (precalc) saved feats to: %s" % feats_file)
...
def calc_test_da_feats():
    ...
    for aug in range(nb_augm):
        ...
        # save
        labels_file = "data/results/resnet_da%d_conv_test_labels.h5" % aug
        feats_file = "data/results/resnet_da%d_conv_test_feats.h5" % aug
        save_array(labels_file, labels)
        save_array(feats_file, feats)
        print("    (precalc) feats: %s" % (feats.shape,))
        print("    (precalc) saved feats to: %s" % feats_file)
...
def train_ensemble():
    ...
    # load validation 
    val_labels = load_array("data/results/resnet_conv_val_labels.h5")
    conv_val_feat = load_array("data/results/resnet_conv_val_feats.h5")
...
def test_ensemble(models):
    ...
    save_array('data/results/resnet_ens_dense_preds.h5', preds)
    return preds 
...
def submit_ensemble(preds):
    ...
    submits_path = 'submits/resnet_ens_dense_preds.gz'
